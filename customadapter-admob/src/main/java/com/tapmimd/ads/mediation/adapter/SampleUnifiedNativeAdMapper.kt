...
//        Log.d("APP@@@","size "+imagess.size)
        ...
        sampleAd.mainImage?.let { mainImage -> images = arrayListOf(SampleNativeMappedImage(mainImage.drawable, mainImage.uri, SAMPLE_SDK_IMAGE_SCALE)) }
        sampleAd.body?.let { body = it }
        sampleAd.icon?.let { iconImage -> icon = SampleNativeMappedImage(iconImage.drawable, iconImage.uri, SAMPLE_SDK_IMAGE_SCALE) }
        ...
        sampleAd.hasVideoContent.let { setHasVideoContent(it) }
        sampleAd.adChoise?.let { adChoicesContent = it }
        sampleAd.mediaView?.let { setMediaView(it) }
        sampleAd.mediaContentAspectRatio.let { setMediaContentAspectRatio(it) }
