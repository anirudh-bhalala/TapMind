...
//        }
    ...
    fun loadInterstitialAd(
        ...
        Log.d(TAG, "Loading ${if (isBiddingAd) "bidding " else ""}interstitial ad: $placementId...")
        ...
        InterstitialAd.load(
            ...
            object : InterstitialAdLoadCallback() {
                ...
                override fun onAdFailedToLoad(loadAdError: LoadAdError) {
                    ...
                    Log.d(
                        TAG,
                        "Interstitial ad ($placementId) failed to load with error: $adapterError"
                    )
...
//    }
    ...
    fun loadRewardedAd(
        ...
        val placementId = parameters.getThirdPartyAdPlacementId()
        val isBiddingAd = TapmindsSdkUtils.isValidString(parameters.getBidResponse())
        Log.d(TAG, "Loading ${if (isBiddingAd) "bidding " else ""}rewarded ad: $placementId...")
        ...
        RewardedAd.load(context, placementId, adRequest, object : RewardedAdLoadCallback() {
            override fun onAdLoaded(ad: RewardedAd) {
                Log.d(TAG, "$TAG1 Rewarded ad loaded: $placementId...")
            ...
            override fun onAdFailedToLoad(loadAdError: LoadAdError) {
                ...
                Log.d(
                    TAG,
                    "$TAG1 Rewarded ad ($placementId) failed to load with error: $adapterError"
                )
...
//    }
    ...
    fun loadNativeAd(
        ...
        val context = activity
        val adRequest =
            createAdRequestWithParameters(isBiddingAd, TapMindAdFormat.NATIVE, parameters, context)

        val nativeAdOptionsBuilder = NativeAdOptions.Builder()
            .setAdChoicesPlacement(getAdChoicesPlacement(parameters))

        // Medium templates can handle multiple images via AdMob's media view
        val template = BundleUtils.getString("template", "", parameters.getServerParameters())
        nativeAdOptionsBuilder.setRequestMultipleImages(template!!.contains("medium"))


        val nativeAdListener = NativeAdListener(parameters, context, listener)
...
//            Log.d(TAG,"Native ad ($placementId) failed to load with error: $adapterError")
    ...
    private inner class TapMindGoogleNativeAd(builder: Builder) : TapMindNativeAd(builder) {
        ...
        override fun prepareForInteraction(
            ...
            if (container is TapMindNativeAdView) {
                val maxNativeAdView = container
                val mainView = maxNativeAdView.mainView
                maxNativeAdView.removeView(mainView)
                nativeAdView?.addView(mainView)
                maxNativeAdView.addView(nativeAdView)

                nativeAdView?.iconView = maxNativeAdView.iconView
                nativeAdView?.headlineView = maxNativeAdView.titleView
                nativeAdView?.advertiserView = maxNativeAdView.advertiserView
                nativeAdView?.bodyView = maxNativeAdView.bodyView
                nativeAdView?.callToActionView = maxNativeAdView.callToActionView

                val mediaView = getMediaVieww()
                when (mediaView) {
    ...
    private fun createAdRequestWithParameters(
        ...
        parameters.isDoNotSell()?.let { doNotSell ->
            ...
            if (doNotSell) {
                networkExtras.putInt("rdp", 1) // Restrict data processing (CCPA)
    ...
    private fun getInlineAdaptiveBannerMaxHeight(parameters: TapMindAdapterParameters): Int {
        ...
        return if ((inlineMaxHeight is Int)) inlineMaxHeight else 0
    }
